...
export default function App() {
  ...
  useEffect(() => {
    setProdutos(lsGet(SK.produtos, []));
    setHistorico(lsGet(SK.historico, []));
    setLocais(lsGet(SK.locais, []));
    setResps(lsGet(SK.responsaveis, []));
    setCategorias(lsGet(SK.categorias, []));
    setPin(localStorage.getItem(SK.pin) || DEFAULT_PIN);
    setLoaded(true);
  }, []);

  // ── Save helpers ──────────────────────────────────────────
  const save = (key, setter, value) => { setter(value); lsSet(key, value); };
  const savePin = (p) => { setPin(p); localStorage.setItem(SK.pin, p); };
  ...
  const mvStep3Confirm = () => {
    setMvError("");
    if (!mvRespInput) { setMvError("Selecione o responsável"); return; }
    const now = new Date().toISOString();
    const evento = { id:Date.now(), code:mvProdObj.code, de:mvProdObj.shelf, para:mvLocalInput, responsavel:mvRespInput, data:now };
    const updatedProd = produtos.map(p => p.code===mvProdObj.code ? {...p, shelf:mvLocalInput} : p);
    const updatedHist = [evento, ...historico];
    save(SK.produtos,  setProdutos,  updatedProd);
    save(SK.historico, setHistorico, updatedHist);
    stopScan();
    setMvStep(1); setMvProdInput(""); setMvProdObj(null);
    setMvLocalInput(""); setMvRespInput(""); setMvError("");
    setShowMove(false);
  ...
  const saveProd = () => {
    ...
    let updated;
    if (apEdit) {
      updated = produtos.map(p => p.code===apEdit.code ? {...p, code, name:apName.trim(), category:apCat, shelf:apLocal} : p);
    } else {
      updated = [...produtos, { code, name:apName.trim(), category:apCat, shelf:apLocal, createdAt:new Date().toISOString() }];
    }
    save(SK.produtos, setProdutos, updated);
  ...
  const deleteProd = (code) => {
    ...
    save(SK.produtos, setProdutos, produtos.filter(p=>p.code!==code));
  ...
  const saveLocal = () => {
    ...
    let updated;
    if (alEdit) {
      updated = locais.map(l => l===alEdit ? v : l);
      save(SK.produtos, setProdutos, produtos.map(p => p.shelf===alEdit ? {...p, shelf:v} : p));
    } else {
      updated = [...locais, v];
    }
    save(SK.locais, setLocais, updated);
  ...
  const deleteLocal = (l) => {
    ...
    save(SK.locais, setLocais, locais.filter(x=>x!==l));
  ...
  const saveResp = () => {
    ...
    save(SK.responsaveis, setResps, arEdit ? resps.map(r=>r===arEdit?v:r) : [...resps,v]);
  ...
  const deleteResp = (r) => {
    ...
    save(SK.responsaveis, setResps, resps.filter(x=>x!==r));
  ...
  const saveCat = () => {
    ...
    save(SK.categorias, setCategorias, acEdit ? categorias.map(c=>c===acEdit?v:c) : [...categorias,v]);
  ...
  const deleteCat = (c) => {
    ...
    save(SK.categorias, setCategorias, categorias.filter(x=>x!==c));
